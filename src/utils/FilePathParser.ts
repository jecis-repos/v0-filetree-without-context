...
export class FilePathParser {
  ...
  private static shouldIncludePath(path: any, options: VisualizationOptions): boolean {
  ...
  private static safeCompare(a: any, b: any): number {
    ...
    if (typeA === "boolean" && typeB === "boolean") return a === b ? 0 : a ? 1 : -1
  ...
  private static sortNodes(nodes: FileNode[], options: VisualizationOptions): void {
    if (!options.sortBy || !Array.isArray(nodes)) return
    ...
    nodes.sort((a, b) => {
      ...
      switch (options.sortBy) {
        case "name":
          comparison = this.safeCompare(a.name, b.name)
          break
        case "size":
          comparison = this.safeCompare(a.size, b.size)
          break
        case "date":
          comparison = this.safeCompare(
            a.lastModified instanceof Date ? a.lastModified.getTime() : 0,
            b.lastModified instanceof Date ? b.lastModified.getTime() : 0,
          )
          break
        case "type":
          comparison = this.safeCompare(a.type, b.type)
          break
      }
  ...
  private static isFilePath(path: any): boolean {
    if (typeof path !== "string") return false
    const fileName = path.split("/").pop() || ""
    return fileName.includes(".") && !fileName.startsWith(".")
  }

  private static getFileExtension(fileName: any): string | null {
